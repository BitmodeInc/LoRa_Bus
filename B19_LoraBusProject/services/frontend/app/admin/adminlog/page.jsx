...
const LogsPage = () => {
  ...
  const [popularSearchLogs, setPopularSearchLogs] = useState([]); // เพิ่ม state สำหรับ popular search logs
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [loraResponse, editResponse, loginResponse, searchResponse, popularResponse] = await Promise.all([
          ...
          fetch('https://service.lora-bus.com/admin_api/getSearchLog/popular'), // ดึงข้อมูล popular search logs
        ]);

        if (!loraResponse.ok || !editResponse.ok || !loginResponse.ok || !searchResponse.ok || !popularResponse.ok) {
          throw new Error('Failed to fetch logs');
        }

        const [loraData, editData, loginData, searchData, popularData] = await Promise.all([
          loraResponse.json(),
          editResponse.json(),
          loginResponse.json(),
          searchResponse.json(),
          popularResponse.json(), // แปลงข้อมูล popular search logs
        ]);
        ...
        setPopularSearchLogs(popularData.data); // ตั้งค่าข้อมูล popular search logs
      } catch (error) {
      ...
      <div className="max-w-6xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-x-auto">
        ...
        <div className="mb-6">
          ...
          >
            ...
            <option value="popular">Popular Search</option> {/* เพิ่มตัวเลือกสำหรับ popular search logs */}
          </select>
        </div>
        ...
        {/* Popular Search Log Section */}
        {selectedLogType === 'popular' && (
          <section>
            <h3 className="text-xl font-semibold mb-2 dark:text-white">Popular Search Data</h3>
            {popularSearchLogs.length === 0 ? (
              <p className="text-gray-500 dark:text-white">No popular search logs found.</p>
            ) : (
              <>
                <table className="table-auto w-full mb-6 border-collapse text-left dark:text-white">
                  <thead>
                  ...
                  <tbody>
                    {popularSearchLogs.map(log => (
                      <tr key={log.Search_Term} className="hover:bg-gray-100 dark:hover:bg-gray-600">
                        <td className="border px-4 py-2">{log.Search_Term}</td>
                        <td className="border px-4 py-2">{log.count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
