...
import { GoogleMap, LoadScript, Marker, Rectangle, InfoWindow,Polyline } from '@react-google-maps/api';
...
const Map = ({ storedResults }) => {
  ...
  const [loRaMarker1, setLoRaMarker1] = useState(null);
  ...
  return (
    ...
    <div className="relative w-full h-[60vh] md:h-[40vh] lg:h-[60vh] xl:h-[60vh] 2xl:h-[60vh] rounded-md overflow-hidden mt-12">
        ...
        <LoadScript googleMapsApiKey="your google map api">
          ...
          {selectedMarker && (
            ...
            >
              <div className="text-black">
                <h4 className="text-lg font-semibold">Name: {selectedMarker.BusStop_Name}</h4>
                <h3 className="text-sm">LAT: {selectedMarker.BusStop_Latitude}</h3>
                <h3 className="text-sm">LNG: {selectedMarker.BusStop_Longitude}</h3>
          ...
          <InfoWindow
            position={{
              lat: parseFloat(selectedLoRa.LoRa_Latitude), // ใช้ตำแหน่งล่าสุดของ marker
              lng: parseFloat(selectedLoRa.LoRa_Longitude),
            }}
            onCloseClick={() => setSelectedLoRa(null)}
          >
            <div className="text-black">
              <h3 className="font-bold">Dip Switch: {selectedLoRa.Dip_Switch_Value}</h3>
              <p>Temperature: {selectedLoRa.LoRa_Temp} °C</p>
              <p>Battery: {selectedLoRa.LoRa_Battery} V</p>
              <p>LAT: {selectedLoRa.LoRa_Latitude}</p>
              <p>LNG: {selectedLoRa.LoRa_Longitude}</p>
              <p>SNR: {selectedLoRa.LoRa_SNR}</p>
              <p>RSSI: {selectedLoRa.LoRa_RSSI} </p>
              <button
